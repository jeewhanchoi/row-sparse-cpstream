    ...
    splatt_global_opts const * const global_options)
{
  ...
  if (rsp_mode) {
    printf("\n\nINFO: Executing Row sparse CP-stream...\n\n");
    factored = cpd.compute_rowsparse(rank, forget, cpd_options, global_options);
  } else {
    printf("\n\nINFO: Executing CP-stream...\n\n");
    factored = cpd.compute(rank, forget, cpd_options, global_options);
  }
