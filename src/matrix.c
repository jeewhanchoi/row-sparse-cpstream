 ...
 * PUBLIC FUNCTIONS
 *****************************************************************************/
void mat_solve_cholesky_with_fallback(
    matrix_t * const A,
    matrix_t * const rhs, bool is_spd)
{
    /* First run Cholesky factorization */
    // bool is_spd;
    // is_spd = mat_cholesky_(A, &is_spdd);
    // /* check dimensions */
    // assert(A->I == A->J);
    ...
    if (is_spd) {
      ...
      fprintf(stderr, "Running SVD solver\n");
      */
      ...
      /*
      fprintf(stderr, "Worksize query complete\n");
      */
      ...
      SPLATT_BLAS(gelss)(&N, &N, &nrhs,
          ...
          work, &lwork, &info);
      /*
      fprintf(stderr, "Actual solving complete\n");
      */

      if(info) {
        fprintf(stderr, "SPLATT: DGELSS returned %d\n", info);
        exit(1);
      }

      else {
        /*
        fprintf(stderr, "\n\n\nActual solving complete\n");
        fprintf(stderr, "SPLATT:   DGELSS effective rank: %d\n", effective_rank);
  ...
  matrix_t const * const A)
{
  ...
  // Create copy of neqs in case it fails
  val_t * A_vals_cpy = (val_t*) malloc(A->I * A->J * sizeof(val_t));
  memcpy(A_vals_cpy, A->vals, A->I * A->J * sizeof(val_t));
    ...
    matrix_t * const B)
{
  ...
  matrix_t * res = mat_alloc(A->I, A->J);
#pragma omp parallel for schedule(static) if(size > 50)
...
/* Subtracts matrix A - B and adds it to matrix dest */
void mat_sub_add(
  matrix_t const * const matA,
  matrix_t const * const matB,
  matrix_t * const dest) {

  assert(matA->I == matB->I);
  assert(matA->J == matB->J);

  idx_t _size = matA->I;

  for (idx_t i = 0; i < _size * _size; i++) {
    // dest->vals[i] += matA->vals[i] - matB->vals[i];
    dest->vals[i] += matA->vals[i] - matB->vals[i];
  }
}
