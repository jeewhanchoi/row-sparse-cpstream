...
/* Check runtime within mttkrp routine */
#include "timer.h"
...
/* 
 * Non omp version for streaming mode
 */
void mttkrp_stream_(
  sptensor_t const * const tt,
  matrix_t ** mats,
  idx_t const mode)
{
  ...
  val_t const * const restrict vals = tt->vals;
    ...
    for(idx_t n=0; n < tt->nnz; ++n) {
      ...
      // val_t * const outrow = outmat + (tt->ind[mode][n] * nfactors);
  ...
  idx_t const mode)
{
  ...
  {
    val_t * restrict accum = splatt_malloc(nfactors * sizeof(*accum));
    val_t * outrow_tmp = splatt_malloc(nfactors * num_threads * sizeof(*outrow_tmp));
    ...
    for(idx_t n=0; n < tt->nnz; ++n) {
      ...
      for (idx_t i=0; i < nfactors * num_threads; i++) {
        outrow_tmp[i] = 0.0;
      }
      ...
      /* write to buf */
      for(idx_t f=0; f < nfactors; f++) {
        idx_t _f = nfactors * tid + f;
        outrow_tmp[_f] += accum[f];
      }
      ...
      // Reduce outrow_tmp among all threads
      if (tid == 0) {
        for(idx_t f = 0; f < nfactors; f++) {
          val_t tmp = 0.;
          for(idx_t t = 0; t < num_threads; t++) {
            tmp += outrow_tmp[t * nfactors + f];
          }
          outrow[f] = tmp;
        }
      }
  ...
  idx_t const mode)
{
  ...
  val_t* outmat_temp = (val_t*) splatt_malloc(num_threads * nfactors * 
                                              sizeof(*outmat_temp));

  #pragma omp for schedule(static)
  for(idx_t x=0; x < nfactors * num_threads; ++x) {
    outmat_temp[x] = 0.;
  }
  ...
  #pragma omp parallel
  {
    ...
    for(idx_t n=0; n < tt->nnz; ++n) {
      ...
      /* write to temp array */
      // idx_t row_ind = tid * I + tt->ind[mode][n];
      val_t* tmp_outrow = outmat_temp + (tid * nfactors);
      for (idx_t f = 0; f < nfactors; f++) {
        tmp_outrow[f] += accum[f];
      }
    }
    ...
    for(idx_t i = 0; i < nfactors; ++i) {
      val_t * const restrict outrow = outmat;
      for(idx_t t = 0; t < num_threads; ++t) {
        outrow[i] += outmat_temp[t * nfactors + i];
      }
    }
    /*
    #pragma omp for schedule(static)
    for(idx_t n=0; n < tt->nnz; ++n) {
      ...
      for(idx_t i = 0; i < nfactors; i++) {
        val_t _accum = 0.0;
        for(idx_t _tid = 0; _tid < num_threads; _tid++) {
          _accum += outmat_temp[(_tid * I + out_ind) * nfactors + i];
        }
        outrow[i] = _accum;
      }
    }
  ...
  idx_t const mode)
{
  ...
  sp_timer_t init_output;
  sp_timer_t write_output;

  timer_fstart(&total_runtime);
  timer_fstart(&init_output);
  */
  // printf("mttkrp_stream,%d,%d\n", mode, I); 
  /* clear output */
  #pragma omp parallel for schedule(static)
  for(idx_t x=0; x < I * nfactors; ++x) {
    outmat[x] = 0.;
